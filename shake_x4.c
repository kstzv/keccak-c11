...
// Four parallel SHAKE instances sharing one interleaved Keccak state
struct shake_ctx {
    uint64_t (*state)[4];
...
void shake_get_third_msg(struct shake_ctx *ctx, const uint8_t *in, uint8_t *out);
void shake_get_fourth_msg(struct shake_ctx *ctx, const uint8_t *in, uint8_t *out);
...
// Clear the 4-way Keccak state and reset the context
void shake_ctx_zero(struct shake_ctx *ctx)
{
...
static void shake_absorb(struct shake_ctx *ctx)
{
    ...
    while (input_counter < ctx->inlen)
    {
        ...
        if (remaining < 8)
        {
            ...
            keccak_xor_bytes((uint8_t *)&ctx->state[output_counter][2], ctx->in[2], remaining);

            keccak_xor_bytes((uint8_t *)&ctx->state[output_counter][3], ctx->in[3], remaining);
        ...
        ctx->state[output_counter][2] ^= load64_le(ctx->in[2]);
        ctx->state[output_counter][3] ^= load64_le(ctx->in[3]);
...
static void shake_finalize(struct shake_ctx *ctx)
{
    ...
    ctx->state[last_word][2] ^= end;
    ctx->state[last_word][3] ^= end;
